...
class WP_Site_Query {
	...
	protected function get_search_sql( $string, $columns ) {
		...
		// Special case: Search for a combination of domain and path.
		if ( strpos( $string, '/' ) ) {
			list( $domain, $path ) = explode( '/', $string, 2 );

			$domain = '%' . $wpdb->esc_like( $domain );
			$path = $wpdb->esc_like( '/' . $path ) . '%';

			$searches[] = $wpdb->prepare( "(domain LIKE %s AND path LIKE %s)", $domain, $path );
		}
